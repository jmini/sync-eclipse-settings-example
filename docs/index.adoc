...
* a git-link:example-use/pom.xml[parent pom file] and several child artifacts:
  ...
  - git-link:example-use/wrong1/pom.xml[wrong1 pom file] (that contains a compile error on purpose)
* another aggregator artifact git-link:example-use/other/pom.xml[other pom file] that has not `parent` as parent pom.

To make the maven structure a little more complex, the parent pom only aggregates `project1` and `project2` and not `wrong1`.
The `other` pom aggregates `parent` and `wrong1`
...
Of course because all projects are really basic, without any dependencies they can be compiled individually.

=== Projects in Eclipse IDE

The artifacts can of course be imported into Eclipse IDE.
Each of them is a valid Eclipse project.
The project metadata were added to the git repository (`.project` and `.classpath` files):
...
The parent project is configured to generate 4 `*.prefs` files inside each `.settings` folder.
See the `pluginManagement` section of the git-link:example-use/pom.xml[parent pom].
...
The corresponding configuration is provided here:
...
As source we are using the files provided by the `org.eclipse.scout.rt-settings` artifact of the Eclipse Scout project.
...
[INFO] ------------------------------------------------------------------------
[INFO]
[INFO] --- eclipse-settings-maven-plugin:3.0.1:eclipse-settings (default-cli) @ parent ---
...
[INFO] ------------------------------------------------------------------------
[INFO]
[INFO] --- eclipse-settings-maven-plugin:3.0.1:eclipse-settings (default-cli) @ project1 ---
[INFO] Project configured.
[INFO]
[INFO] ------------------------------------------------------------------------
...
[INFO] ------------------------------------------------------------------------
[INFO]
[INFO] --- eclipse-settings-maven-plugin:3.0.1:eclipse-settings (default-cli) @ project2 ---
[INFO] Project configured.
[INFO] ------------------------------------------------------------------------
[INFO] Reactor Summary:
[INFO]
...
[INFO] ------------------------------------------------------------------------
[INFO]
[INFO] --- eclipse-settings-maven-plugin:3.0.1:eclipse-settings (default-cli) @ other ---
